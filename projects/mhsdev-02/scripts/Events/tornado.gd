...
func _select_target():
	...
	target_pos = _get_random_direction() * 100 + start_pos
...
func _process(delta):
	...
	if current_cooldown < move_cooldown: # Don't move during cooldown period
		tornado_body.velocity = tornado_body.global_position.direction_to(target_pos) * delta * 2000
		tornado_body.move_and_slide()

func _ready() -> void:
	# Set default stats and connections
	collider.connect("body_entered", fling)
	push_strength = EventMan.scale_val(data.strength) * 300 # Use scale_val for logarithmic growth
	tornado.scale = _double_vec2(EventMan.scale_val(data.strength))
	move_cooldown = 2 / data.strength
